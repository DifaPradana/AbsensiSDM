...
new class extends Component
{
    ...
    public function render()
    {
        ...
        $query = IzinAbsen::with(['user', 'hrd', 'direktur'])
            ->whereHas('user')
            ->latest();

        // HRD hanya lihat tipe cuti untuk multi-approval
        // izin & sakit tetap bisa diapprove langsung
        if ($role === 'hrd') {
            $query->where(function ($q) {
                $q->where('tipe_izin', 'cuti')
                    ->orWhereIn('tipe_izin', ['izin', 'sakit']);
            });
        }
        ...
        if ($this->search) {
            $query->whereHas(
                'user',
                fn($q) => $q->where('nama_karyawan', 'like', '%' . $this->search . '%')
            );
        }
                ...
                <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                    ...
                    <div class="flex items-center justify-between p-4 gap-3 flex-wrap">
                        <div class="flex gap-2 flex-wrap flex-1">
                            ...
                            {{-- Filter Status — opsi sesuai enum baru --}}
                            <div class="relative">
                                <select
                        ...
                        <table class="w-full border border-gray-300 text-sm text-left">
                            <thead class="text-xs text-gray-700 uppercase bg-gray-100">
                                <tr>
                                    ...
                                    <th class="border px-4 py-3 text-center">Dokumen</th>
                                    <th class="border px-4 py-3 text-center">Note</th>
                                    <th class="border px-4 py-3 text-center">Status</th>
                                    <th class="border px-4 py-3 text-center">Review</th>
                            ...
                            <tbody>
                                ...
                                ? \Carbon\Carbon::parse($izin->mulai_izin)->diffInDays($izin->akhir_izin) + 1
                                : 1;

                                $tipeBadge = match(strtolower($izin->tipe_izin ?? '')) {
                                ...
                                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50">
                                    ...
                                    {{-- Kolom review: tampilkan siapa & kapan approve/reject --}}
                                    <td class="border px-4 py-3 text-center text-xs text-gray-600">
                                        @if($izin->hrd_id)
                                        <div class="mb-1">
                                            <span class="font-semibold">HRD:</span>
                                            {{ $izin->hrd->nama_karyawan ?? '-' }}<br>
                                            <span class="text-gray-400">
                                                {{ \Carbon\Carbon::parse($izin->hrd_at)->translatedFormat('d F Y') }}
                                            </span>
                                            @if($izin->hrd_note)
                                            <br><em class="text-gray-500">"{{ $izin->hrd_note }}"</em>
                                            @endif
                                        </div>
                                        @endif
                                        @if($izin->direktur_id)
                                        <div>
                                            <span class="font-semibold">Direktur:</span>
                                            {{ $izin->direktur->nama_karyawan ?? '-' }}<br>
                                            <span class="text-gray-400">
                                                {{ \Carbon\Carbon::parse($izin->direktur_at)->translatedFormat('d F Y') }}
                                            </span>
                                            @if($izin->direktur_note)
                                            <br><em class="text-gray-500">"{{ $izin->direktur_note }}"</em>
                                            @endif
                                        </div>
                                        @endif
                                        @if(!$izin->hrd_id && !$izin->direktur_id)
                                        <span class="text-gray-400">-</span>
                                        @endif
                                    </td>

                                    <td class="px-4 py-3 text-center">
                                        {{-- Tombol edit hanya tampil kalau masih bisa diaksi --}}
                                        @if(
                                        ($currentRole === 'hrd' && $izin->status === 'menunggu_hrd') ||
                                        ($currentRole === 'direktur' && $izin->status === 'menunggu_direktur')
                                        )
                                        <button
                                            type="button"
                                ...
                                {{-- Modal Dokumen --}}
                                @if($izin->dokumen_izin)
                                <div class="modal fade" id="modalDokumen{{ $izin->izin_id }}"
                                    ...
                                    <div class="modal-dialog modal-dialog-centered modal-lg">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body text-center">
                                                ...
                                                <img src="{{ asset('storage/'.$izin->dokumen_izin) }}"
                                                    class="img-fluid rounded">
                                                @elseif($ext === 'pdf')
                                                <iframe src="{{ asset('storage/'.$izin->dokumen_izin) }}"
                                                    width="100%" height="600"></iframe>
                                                @else
                                                <a href="{{ asset('storage/'.$izin->dokumen_izin) }}"
                                ...
                                @empty
                                <tr>
                                    <td colspan="10" class="border px-4 py-6 text-center text-gray-400 italic">
                                        Tidak ada data izin.
                                    </td>
                                </tr>
                                @endforelse
